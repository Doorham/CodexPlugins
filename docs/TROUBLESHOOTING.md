...
“配置系统代理”只在 Codex 尚未启用 `respect_system_proxy` 时出现；配置完成后会变成不可点击的“已配置”。它不能证明代理链路、服务端或本地网络都正常。请运行“检测连接”，以本次 HTTP 101 握手作为判断依据；如果失败，记录状态与耗时并排查当前代理线路，不要反复配置，也不要发送 `.codex` 全目录、对话数据库或代理订阅。

## Updream 剪贴板助手没有动作

确认助手已开启，并且复制内容确实来自支持的 Updream 画布格式。普通图片、文本、JSON、文件拖放或无效 PNG 会被有意忽略。该行为是安全边界，不代表程序故障。

如果复制 Updream 内容后尚未粘贴、只切回 Photoshop 就出现“不能输入剪贴板，因为程序错误”，请确认版本至少为 Updream Clipboard Cleaner 1.0.4。该版本会把目标内容严格转换为单一 24 位 `CF_DIB`；更新后需重新从 Updream 复制一次，旧剪贴板不会被再次处理。
...
编辑器和 Python 分别安装成功，不代表编辑器已经能够使用 Python。请在报告中确认被识别的路线：VS Code/Cursor 通常需要 Python 扩展，Visual Studio 需要对应工作负载，PyCharm 需要可用解释器。该模块只报告，不会自动下载大型软件、工作负载或扩展。

## 环境补全按钮没有执行操作

当清单内项目全部合格时，按钮会显示“已配置齐全”，点击不会重复安装。若仍显示“一键自动补全”但失败，请保留界面错误详情，重点检查管理员权限、`winget`、Python/PATH、代理和企业策略。不要通过关闭 Codex、VPN、Clash Verge 或 Aurora 来试错。

UTF-8 修改成功后必须重启 Windows 才能完全生效。需要退回原代码页时，使用工具提供的恢复入口；恢复前也会建立安全快照并要求重启。
