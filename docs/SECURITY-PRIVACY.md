...
CodexPlugins 是公开可审计的开源项目。Public 意味着任何人都可以读取和复制已经推送的 Git 历史，因此任何密码、验证码、Token、密钥、私人配置或内部数据都不得进入提交。
...
- 公开 clone/pull 不要求 GitHub 登录。
- 参与者推送代码时，推荐使用 Git Credential Manager 的浏览器授权，或由本人管理的 SSH 密钥。
...
发现秘密已经提交时，不要只在下一次提交中删除。应立即停止推送、撤销或轮换秘密，再由维护人员清理尚未公开的 Git 历史；如果已推送到 Public 仓库，应假定内容已经被第三方复制，并按泄露事件处理。
